#!/usr/bin/python3
"""
User Model: This script defines the User class for managing user data.
"""
...
class User():
    ...
    __password = None  # Initialize the private password attribute as None

    def __init__(self):
        """
        Initialize a new user:
        - assigned an unique `id` using UUID (Universally Unique Identifier).
        """
    ...
    def password(self):
        """
        Password getter: Returns the hashed password.
        """
    ...
    def is_valid_password(self, pwd):
        """
        Valid password:
        - `False` if `pwd` is `None`, not a string, or `__password` is `None`
        - Compare `__password` and the MD5 hash value of `pwd`
        """
        if pwd is None or type(pwd) is not str:
            return False
